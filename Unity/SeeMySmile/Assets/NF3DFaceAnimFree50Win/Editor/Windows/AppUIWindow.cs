...
namespace Assets.Scripts.NFEditor
{
    ...
    public class AppUIWindow : EditorWindow
	{
        ...
        private const string NotAvatarMsg = "An avatar has not been created yet. ";
        ...
        private static WizardOverview _wizardOverview;
        ...
        internal static void CreateWizardOverview()
        {

            try
            {
                if (_wizardOverview != null)
                {
                    if (AllWindows.Contains(_wizardOverview))
                    {
                        AllWindows.Remove(_wizardOverview);
                    }
                    _wizardOverview.Close();
                ...
                _wizardOverview = GetWindowWithRect<WizardOverview>(new Rect(recLeft, recTop, recWidth, recHeight), true, "");

                AllWindows.Add(_wizardOverview);
                WizardController.ActiveWindow = _wizardOverview;
            }
            catch (Exception exp)
            {
                Debug.LogError(exp.Message);
	    ...
	    private static void DrawHelpMenu()
	    {
            ...
            if (GUI.Button(new Rect(172, 2, 50, 20), new GUIContent("Help")))      
            {
	    ...
	    {
	        if (GUI.Button(new Rect(102, 2, 70, 20), new GUIContent("Animation")))  
            {
        ...
        private static void DrawAudioMenu()
        {
            if (GUI.Button(new Rect(52, 2, 50, 20), new GUIContent("Audio")))
            {
        ...
        private static void OnGetReady()
        {
            try
            {
                Controller.ReadyToProceed = false;
                ...
                WizardController.ActiveWindow = wizWnd;
